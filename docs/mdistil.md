...
$$\begin{align} C_2(t,\vec{p}) = \sum_{\alpha,n,d}  B^{[n_1,d_1;n_2,d_2;n_3,d_3]}_\alpha(v^{1,A},v^{2,B},v^{3,C};t,\vec{p})  \Big( & \delta_{ABC}^{\bar{ABC}} B^{[n_1,d_1;n_2,d_2;n_3,d_3]}_\alpha(w^{1,\bar{A}},w^{2,\bar{B}},w^{3,\bar{C}};t,\vec{p}) \\
 & + abc \Big) \end{align}$$
